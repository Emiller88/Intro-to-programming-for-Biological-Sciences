// HW 4
// Edmund Miller
// Calculates Height from sequence file and outputs to file and a window using nCurses
...
int main()
{
   ifstream seqfile;
   int  seqLength = 0;
   double count = 0, NT_c[4][10]={{0},{0},{0},{0}}, I[10]={0}, H[10]={0}, heights[4][5];
   ...
   getFileInput(seqfile, filename, count, NT_c, seqLength);//Get seq from inputfile
...
void calcEntAndHeight(int seqLength, double count, double NT_c[4][10],
                    double H[], double I[], double heights[4][5], string filename2)
{
...
void displayScreen(int seqLength, double heights[4][5])
{
    ...
    for (int j=0; j < seqLength; j++)
    {
        ...
        displayG(heights[1][j], height - 5 - lastHeight, marginWidth + j * boxWidth + 1);
        lastHeight += calcLetterSpacing(heights[1][j]);

        displayC(heights[2][j], height - 5 - lastHeight, marginWidth + j * boxWidth + 1);
        lastHeight += calcLetterSpacing(heights[2][j]);

        displayU(heights[3][j], height - 5 - lastHeight, marginWidth + j * boxWidth + 1);
