...
int main()
{
  ...
  if (userNum < 0 || userNum >= tRNA_vec.size())//Check to make sure users number is valid
  {
...
//Reads through file and stores in a vector of Name and Sequence to then be stored in a large vector of vectors
void readFile(fstream & inFile, vector<vector<string> > & tRNA_vec, int & seqCount)
{
  ...
  while (!inFile.eof())
  {
    ...
    else if(fileInput.length() != seqLength)//Checks to see if the current sequence matches the first sequence length
    {
      cout << "Error in sequence " << (seqCount + 1) << "(" << fileVec[0] << "):  sequences must be of same length" << endl;
      exit(0);
    }
    fileVec.push_back(fileInput);
...
void outputSeq(int userNum, int seqCount, vector<vector<string> > tRNA_vec)
{
  cout << "The name of your molecule is:" << endl
    ...
    << "There are a total of " << tRNA_vec.size() << " sequences in your alignment" << endl;
}