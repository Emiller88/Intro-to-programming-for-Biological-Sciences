...
int main()
{
    ...
    do
    {
        ...
        do
        {
            cout << "Would you like to enter new sequences? type 'y' for yes and 'n' for no" << endl;
            cin >> yesOrNo;
...
void checkInput(char seqBP, bool & dataValid)
{
    ...
    if (seqBP != 'A' && seqBP != 'T' && seqBP != 'G' && seqBP != 'C' && seqBP != 'U')
    {
...
float calculateSim(float simBP, float length)
{
    return (simBP / length);
}
...
{
    cout << "The sequence identity between sequence 1 and 2 is:" << endl << iD12 << endl;
}
